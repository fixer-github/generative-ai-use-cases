...
const defaultConfig: MermaidConfig = {
  ...
  securityLevel: 'strict', // Enhanced security - prevent script execution
      ...
      try {
        ...
        if (svgElement) {
          ...
          const sanitizedSvg = DOMPurify.sanitize(svgElement.outerHTML, {
            ...
            ALLOWED_ATTR: [
              'x', 'y', 'x1', 'y1', 'x2', 'y2', 'width', 'height', 'cx', 'cy', 
              'r', 'rx', 'ry', 'fill', 'stroke', 'stroke-width', 'stroke-dasharray',
              'transform', 'd', 'points', 'viewBox', 'class', 'id', 'style',
              'opacity', 'fill-opacity', 'stroke-opacity', 'clip-path', 'mask',
              'marker-start', 'marker-mid', 'marker-end', 'href'
            ],
        ...
        console.error(error);
        const errorMessage = DOMPurify.sanitize(`<div>${t('diagram.invalid_syntax')}</div>`);
        setSvgContent(errorMessage);
      }
    }
  }, [code, t]);
