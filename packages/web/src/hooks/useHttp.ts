...
const billingFetcher = (url: string) => {
  return billingApi.get(url).then((res) => res.data);
};
...
/**
 * Hooks for Billing API Http Request
 * Same as useHttp but uses billingApi instance
 * @returns
 */
const useBillingHttp = () => {
  return {
    api: billingApi,
    ...
    get: <Data = any, Error = any>(
      ...
      // eslint-disable-next-line react-hooks/rules-of-hooks
      return useSWR<Data, Error>(url, billingFetcher, config);
    },
    ...
    getPagination: <Data = any, Error = any>(
      ...
      // eslint-disable-next-line react-hooks/rules-of-hooks
      return useSWRInfinite<Data, Error>(getKey, billingFetcher, config);
    },
...
export { useBillingHttp, usePagination, useSwrWithFetcher, useSwrWithAPI };
