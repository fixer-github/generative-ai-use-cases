...
import { createS3Client, createDynamoDBClient } from '../utils/awsClientFactory';

export const useTenantResources = () => {
  const { credentials, isLoading: credentialsLoading, error: credentialsError } = useSTSCredentials();
   ...
   */
  const listS3Objects = useCallback(async (bucketPrefix: string, tenantId: string) => {
    if (!credentials) {
   ...
   */
  const queryDynamoDB = useCallback(async (
    tablePrefix: string,
    tenantId: string,
    ...
    try {
      ...
      const errorMessage = err instanceof Error ? err.message : 'Failed to query table';
      setError(errorMessage);
      throw err;
    } finally {
      setIsLoading(false);
