...
/**
 * Fetch full conversation history from DynamoDB with pagination
 * Retrieves all messages for a given chat in chronological order
 * TODO: Add token limit handling to prevent exceeding model context window
 */
async function fetchFullConversation(
  ...
  // Messages are already in chronological order (ScanIndexForward: true)
  return allMessages;
}

/**
 * Detect if this is an empty conversation start
 * Returns true if body.content is empty/whitespace
 * This signals "start new conversation" intent
 */
function isEmptyConversationStart(
  body: CreateAssistantMessageRequest
): boolean {
...
function convertToBedrockMessages(
  ...
  const bedrockMessages = messages
    ...
    .map((msg) => ({
      ...
      content: [{ text: msg.content }] as [{ text: string }],
    }));

  // Ensure last message is user role (Bedrock requirement)
  if (
    bedrockMessages.length > 0 &&
    bedrockMessages[bedrockMessages.length - 1].role !== 'user'
  ) {
    console.warn(
      'Last message in conversation is not user role - Bedrock requires user as last message'
    );
  }
...
async function handleCreateMessage(
  ...
  // Check if this is an empty conversation start
  if (isEmptyConversationStart(body)) {
    // Empty message signals "start new conversation"
    if (!chatId) {
      chatId = crypto.randomUUID();
    }
    const cleanChatId = chatId.replace('chat#', '');

    // Get assistant to retrieve name for chat creation
    ...
    if (!canAccessAssistant(assistant, userId, event)) {
      return forbidden403Response({ message: 'Access denied' });
    }

    // Create chat history entry for new conversation
    ...
    return ok200Response({ chatId: cleanChatId });
  }
  ...
  // Fetch full conversation history if this is not a new conversation
  let conversationHistory: AssistantMessage[] = [];
  if (!isNewConversation) {
    conversationHistory = await fetchFullConversation(
      userId,
      cleanChatId,
      event
    );
  ...
  const bedrockMessages = convertToBedrockMessages(conversationHistory);
  ...
  // Call Bedrock with assistant configuration and full conversation history
