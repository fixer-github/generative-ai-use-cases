...
/**
 * Determine the appropriate video bucket for a tenant
 * Returns shared temp bucket for default tenant, tenant-specific bucket for others
 */
export async function getVideoBucketForTenant(
...
/**
 * Determine the appropriate video bucket for storing job output
 * Used in job creation - returns default bucket for default tenant, tenant bucket for others
 */
export async function getVideoBucketForJobOutput(
  tenantId: string | undefined,
  defaultBucket: string
): Promise<string> {
  if (!tenantId || isDefaultTenant(tenantId)) {
    console.log(`Using default video bucket: ${defaultBucket}`);
 ...
 * Returns different bucket configurations for default vs tenant users
 */
export async function getVideoBucketConfigForCopy(
  tenantId: string | undefined,
  region: string,
  defaultBucket: string
): Promise<{
