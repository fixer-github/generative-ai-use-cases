...
import { createS3Client, uploadFiles } from './import/s3Upload';
...
program
  ...
  .option('--local-files <path>', 'ローカルバックアップディレクトリ')
  .requiredOption('-r, --region <region>', 'AWS リージョン')
  .option('-p, --profile <profile>', 'AWS プロファイル')
  .option('-d, --dry-run', 'ドライランモード')
  .option('-c, --concurrency <num>', 'S3 アップロード同時実行数', '10')
  .action(async (options) => {
    ...
    try {
      ...
      // S3 ファイルアップロード（オプション）
      if (options.s3Mappings && options.targetBucket && options.localFiles) {
        console.log('\n--- S3 ファイルアップロード ---');
        ...
        if (uploadStats.errors.length > 0) {
          ...
          uploadStats.errors.slice(0, 5).forEach((e) => console.log(`  - ${e}`));
        }
      }
