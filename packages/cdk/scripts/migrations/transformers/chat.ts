...
import { extractTenantFromUserId } from './assistant';
...
export type ChatMessage = {
  ...
  assistantId?: string; // Only set if chat is linked to an assistant
  chatId: string;
  userId: string;
  role: 'user' | 'assistant';
  content: string;
...
export function transformConversationToChat(
  ...
  // PK may already have "user#" prefix or be raw, normalize it
  const normalizedPK = oldConversation.PK.startsWith('user#')
    ? oldConversation.PK
    : `user#${oldConversation.PK}`;

  const userId = normalizedPK;

  // Determine tenant ID - extractTenantFromUserId expects userId format
  // Pass the raw PK to extractTenantFromUserId which handles "tenant#xxx#user#yyy" pattern
  const tenantId = options.tenantExtractor
    ? options.tenantExtractor(oldConversation.PK)
    : extractTenantFromUserId(oldConversation.PK, options.defaultTenantId);
...
export async function transformConversationMessages(
  ...
  for (const { messageId: oldMessageId, message } of flatMessages) {
    ...
    // Only set assistantId if chat is linked to an assistant
    if (chat.assistantId) {
      chatMessage.assistantId = chat.assistantId;
    }
