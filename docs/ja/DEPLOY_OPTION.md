...
`selfSignUpTenantMap` を利用して、サインアップしたユーザーにメールアドレスから自動的にテナントIDを割り当てることができます。

マップは `tenantId` と任意の `domains` または `emails` から構成されます。ユーザーがサインアップすると、入力されたメールアドレスがマップと照合されます。

- メールアドレスまたはそのドメインが一致した場合、対応する `tenantId` が割り当てられます。
