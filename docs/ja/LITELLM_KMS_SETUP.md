...
│                                      │ │  (APIキー)      │
│  1. LITELLM_CONFIG環境変数を読み込み  │◀┤                 │
│  2. Secrets ManagerからAPIキー取得   │ │  ┌───────────┐  │
│  3. ランタイム設定を動的に生成        │ │  │    KMS    │  │
│  4. OpenAI互換APIを提供              │ │  │  (暗号化)  │  │
└──────────────────────────────────────┘ │  └───────────┘  │
                    │                    └─────────────────┘
                    ▼
            ┌──────────────┐
...
1. **設定**: `cdk.json`でプロバイダーとモデルを定義
2. **デプロイ**: CDKがKMSキー、Secrets Managerエントリー、LiteLLM Proxy Serverを作成
3. **実行時**: プロキシサーバー起動時：
   - `LITELLM_CONFIG`環境変数から設定を読み込み
   - `config_loader.py`がAWS Secrets ManagerからAPIキーを取得
   - ランタイム設定を動的に生成（ファイルなし）
   - 動的設定でLiteLLMを起動
