  ...
  useEffect(() => {
    Browser.storage.local.get(SETTINGS_KEY).then((value) => {
      const identityPoolId =
        value[SETTINGS_KEY]?.identityPoolId ??
        import.meta.env.VITE_APP_IDENTITY_POOL_ID;
      const lambdaArn =
        value[SETTINGS_KEY]?.lambdaArn ??
        import.meta.env.VITE_APP_PREDICT_STREAM_FUNCTION_ARN;
      const userPoolClientId =
        value[SETTINGS_KEY]?.userPoolClientId ??
        import.meta.env.VITE_APP_USER_POOL_CLIENT_ID;
      const userPoolId =
        value[SETTINGS_KEY]?.userPoolId ??
        import.meta.env.VITE_APP_USER_POOL_ID;
      const region =
        value[SETTINGS_KEY]?.region ?? import.meta.env.VITE_APP_REGION;
      const apiEndpoint =
        value[SETTINGS_KEY]?.apiEndpoint ??
      ...
      const cognitoDomain =
        value[SETTINGS_KEY]?.cognitoDomain ??
        import.meta.env.VITE_APP_SAML_COGNITO_DOMAIN_NAME;
